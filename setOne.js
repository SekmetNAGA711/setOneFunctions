...
// CODE HERE
let oneLine =(parameter1, parameter2) => console.log(`'The first parameter is ${parameter1}. The second parameter is ${parameter2}`)

// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
oneLine('candy', 'ice cream')
...
let greeting = (firstName, lastName, callback) => {
    callback(firstName + ' ' + lastName);
}

// INVOKE 'GREETING FUNCTION' HERE
greeting('Nana', 'Acheampong', (fullName)=> {
...
function pricesPlusTax (arr, call) {
    for(i =0; i<arr.length; i++){
      totalCost.push(arr[i] *.20 + arr[i])
    }
 call(totalCost)
}
